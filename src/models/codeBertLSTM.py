...
#region model 
class CodeBERTLSTMClass(torch.nn.Module):
    def __init__(self, num_classes, hidden_dim=128, n_layers=2, bidirectional=True, dropout=0.3):
        super(CodeBERTLSTMClass, self).__init__()
        self.codebert = AutoModel.from_pretrained('microsoft/codebert-base', cache_dir="./cache")
        self.lstm = nn.LSTM(self.codebert.config.hidden_size, hidden_dim, num_layers=n_layers, bidirectional=bidirectional, dropout=dropout, batch_first=True)
    ...
    def forward(self, input_ids, attention_masks):
        ...
        embedding = embedding.view(batch_size, num_chunks, -1, self.codebert.config.hidden_size)
        
        lstm_out, (hidden, cell) = self.lstm(embedding.mean(1))
...
model = CodeBERTLSTMClass(NUM_CLASSES)
model.to(DEVICE)
